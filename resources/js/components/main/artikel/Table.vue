...
export default {
  ...
  methods: {
    getArtikels() {
      let self = this;
      self.$store.dispatch("getArtikels").then((response) => {
        self.artikels = response.data;
      });
    },
    ...
    deleteArtikel() {
      ...
      if (!self.isRequest && self.key_word === "DELETE") {
        ...
        self.$store
          .dispatch("deleteArtikel", self.artikel.id)
          .then((response) => {
            self.key_word = "";
            self.deleteCurrentArtikel();
            flash(response);
