  ...
  <v-card>
    <v-card-title>
      <div class="text-center mt-4">
        <v-btn rounded color="primary" dark @click="add()">Create FAQ</v-btn>
      </div>
      <!--======================================================================================
          SEARCH 
      ==========================================================================================-->
...
export default {
  data() {
    return {
      ...
      headers: [
        ...
        { text: "Jabawan", value: "answer_indo" },
  ...
  methods: {
    getFaqs() {
      let self = this;
      self.$store.dispatch("getFaqs").then((response) => {
    ...
    deleteFaq() {
      ...
      if (!self.isRequest && self.key_word === "DELETE") {
        ...
        self.$store
          .dispatch("deleteFaq", self.faq.id)
          .then((response) => {
            self.key_word = "";
            self.deleteCurrentFaq();
            flash(response);
    ...
    deleteCurrentFaq() {
      let self = this;
      let index = _.findIndex(self.faqs, function (faq) {
        return faq.id === self.faq.id;
      });
