        ...
        >
            ...
            <template v-slot:[`item.action`]="{ item }">
                ...
                <v-icon
                    ...
                    class="mr-2"
                    @click="download(item)"
                    v-if="item.jenis_pengajuan_id != 1"
                >
                    mdi-cloud-download
                </v-icon>
            </template>
            <template v-slot:no-data>
                <v-btn sm outlined color="indigo" style="border: 0">
                    No data
                </v-btn>
            </template>
        </v-data-table>
...
export default {
    ...
    methods: {
        ...
        download(item) {
            console.log(item, window.location.host);
            let url = "";
            if (item.jenis_pengajuan_id == 3) {
                url =
