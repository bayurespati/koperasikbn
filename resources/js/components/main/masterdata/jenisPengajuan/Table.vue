...
export default {
  ...
  methods: {
    getJenisPengajuan() {
      let self = this;
      self.$store.dispatch("getJenisPengajuan").then((response) => {
        self.jenisPengajuans = response.data;
      });
    },

    deleteJenisPengajuan() {
      let self = this;
      if (!self.isRequest && self.key_word === "DELETE") {
        self.isRequest = true;

        self.$store
          .dispatch("deleteJenisPengajuan", self.jenisPengajuan.id)
          .then((response) => {
            self.key_word = "";
            self.deleteCurrentJenisPengajuan();
            flash(response);
    ...
    deleteCurrentJenisPengajuan() {
      let self = this;
      let index = _.findIndex(self.jenisPengajuans, function (jenisPengajuan) {
        return jenisPengajuan.id === self.jenisPengajuan.id;
      });

      self.jenisPengajuans.splice(index, 1);
    },

    openDialog(item) {
      this.jenisPengajuan = item;
      this.dialog = true;
    },
