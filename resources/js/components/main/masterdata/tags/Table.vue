  ...
  <v-card>
    <v-card-title>
      <div class="text-center mt-4">
        <v-btn rounded color="primary" dark @click="add()">Create Tag</v-btn>
      </div>
      <!--======================================================================================
         SEARCH 
      ==========================================================================================-->
...
export default {
  ...
  methods: {
    getTags() {
      let self = this;
      self.$store.dispatch("getTags").then((response) => {
    ...
    deleteTag() {
      ...
      if (!self.isRequest && self.key_word === "DELETE") {
        ...
        self.$store
          .dispatch("deleteTag", self.tag.id)
          .then((response) => {
            self.key_word = "";
    ...
    deleteCurrentTag() {
      let self = this;
      let index = _.findIndex(self.tags, function (tag) {
        return tag.id === self.tag.id;
      });
