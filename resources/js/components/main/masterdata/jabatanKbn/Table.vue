  ...
  methods: {
    ...
    deleteJabatanKbn() {
      ...
      if (!self.isRequest && self.key_word === "DELETE") {
        ...
        self.$store
          .dispatch("deleteJabatanKbn", self.jabatanKbn.id)
          .then((response) => {
            self.key_word = "";
            self.deleteCurrentJabatanKbn();
            flash(response);
    ...
    deleteCurrentJabatanKbn() {
      let self = this;
      let index = _.findIndex(self.jabatanKbns, function (jabatanKbn) {
        return jabatanKbn.id === self.jabatanKbn.id;
      });

      self.jabatanKbns.splice(index, 1);
    },

    openDialog(item) {
      this.jabatanKbn = item;
      this.dialog = true;
    },
